...
// isSafe2 the same as isSafe, but this one tolerates one bad level
// We do it by evaluating the correctnes of all resulting slices after
...
func isSafe2(r []int) bool {
	...
	for i := 0; i < len(r); i++ {
		if isSafe(slices.Concat(r[:i], r[i+1:])) {
			return true
		}
	}
